...
// generate wave heightfield at time t based on initial heightfield and dispersion relationship根据初始高度场和色散关系，生成t时刻的波高度场
                                       ...
                                       float patchSize)
{
    ...
    if ((x < out_width) && (y < out_height))
    {
        ...
        // output frequency-space complex values输出频率空间复数值
        ht[out_index] = complex_add(complex_mult(h0_k, complex_exp(w * t)), complex_mult(conjugate(h0_mk), complex_exp(-w * t)));
        //ht[out_index] = h0_k;
    }
}

// update height map values based on output of FFT根据FFT的输出更新高度图值
...
// generate slope by partial differences in spatial domain通过空间区域的部分差异生成斜率
